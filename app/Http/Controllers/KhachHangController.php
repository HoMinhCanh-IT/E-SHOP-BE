...
class KhachHangController extends Controller
{
    public function logout(Request $request)
    {
        ...
        if ($tai_khoan_dang_dang_nhap) {
            // Hủy token xác thực của người dùng để đăng xuất
            $tai_khoan_dang_dang_nhap->tokens->each(function ($token) {
                $token->delete();  // Xóa tất cả token của người dùng
            });
    ...
    public function getAvatar(Request $request)
    {
        ...
        $file_extension = $file->getClientOriginalExtension(); // lay extension file
